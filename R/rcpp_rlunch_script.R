...
for(i in seq(6)){
  ...
  print(paste("completed size:", size))
  beepr::beep()
}
...
mat_mean_perf = matrix(ncol = 6, nrow= 5)
for(i in seq(5)){
...
par(mar=c(5,5,1,1))
plot(x = 10^seq(5), y = mat_mean_perf[,2], 
     log="x", type = "b", ylim = c(1, 1.5e8), xlab = "Sample size",
...
lines(x = 10^seq(5), y = mat_mean_perf[,1], type ="b",  col = "darkorange", pch = 16)
lines(x = 10^seq(5), y = mat_mean_perf[,2], type ="b",  col = "darkblue", pch = 16)
