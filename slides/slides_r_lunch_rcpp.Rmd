...
- `Python` via `rPython`, `rJython` or `XRPython`
- `Julia` via `XRJulia`
- `JavaScript` via `V8`
- `Excel`, `JSON`, `SQL`, `Perl`, ...
...
no gain in performance, bug when running
...
- There exists various ways to write an algorithm in `R`, and depending on the syntax, the object created and the functions called, different approach will lead to different execution time.
...
- `Rcpp` is an interface that enables you to write, compile and execute `C++` code inside of an `R` environment with a lighter syntax than classical `C++`. `Rcpp` provides already implemented classes and `R`-like functions for an easier development.
