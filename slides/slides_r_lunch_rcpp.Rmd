...
An algorithm to solve this problem could be
```{r, eval = FALSE}
<<<<<<< HEAD
choose n and set
x <- 1.0
...
pi = 1.0
choose n
>>>>>>> 8a0527d944a9ab48ac1648898583c4c4426b3564
for k from 1 to n
  x <- -x
  pi <- pi + x / (2.0 * k + 1.0)
return pi <- 4.0 * pi
...
For this particular task, we can compare the performances of several programming languages.
...
`C++` is a general-purpose programming language developed initially as an extension of the `C` language that included classes for object-oriented programming. As opposed to `R` or `python`, `C++` is a compiled language. `C++` benefit of fast execution time and is considered as a benchmark for speed as it is comparatively as fast as most lower level language such as `Fortran` or `C`.
...
Hence, there exists different way to script a function in `R`, and depending on the functions and objects defined, different implementation will have different computation time.
...
TO illustrate this notion, l^et us consider a simple example where we want to construct a vector of length $n$.
...
| Matrix constructor |`X = matrix(0,10,10)` |`NumericMatrix X(5, 5);`|
...
The `Rcpp` library provide a variety of `R`-like functions. This set of functions is defined as `Rcpp sugar`. It provides vectorized procedures, functions for strings, statistical functions and other useful `R`-like functions.
...
`R` is know to be particularly inefficient with iterative loop such as `for` and `while` loop.
...
In addition to the functions provided by `Rcpp` and the `C++` standard library `std::`, one can also make use of `C++` linear algebra libraries such as `Armadillo` and `Eigen` with respectively `R` package `RcppArmadillo` and `RcppEigen`. Such libraries provides high-level syntax and efficient functions to work with data that can be stored in vectors, matrices, cubes, dense and sparse matrices. We present here how to use the `Armadillo` library. 
...
We here consider a real case application of rewriting `R` code in `C++` code.
For this example, we will consider a naive implementation of an estimator of the **Haar Wavelet Variance**.

This statistical quantity is used in the context of inference of time series models. Indeed, let $F_{\boldsymbol{\theta}}, \boldsymbol{\theta} \in \Theta \subseteq \mathbb{R}^{p}$ be the true data generating process of a univariate time series $\left\{Y_{t} ; t \in \mathbb{Z}\right\}$, the Generalized Method of Wavelet Moment (GMWM) estimator,  $\boldsymbol{\hat{\theta}}$,  exploits the mapping between $\boldsymbol{\theta}$ and the Wavelet Variance $\boldsymbol{\nu}$ by minimizing the distance between the empirical and estimated Wavelet Variance. 
...
A basic algorithm to compute an estimator of this quantity provide an interesting use case as it provide an example on how to mix the use of different `Rcpp` functions and `arma::` classes.
...
<div align = "center">
<img src="assets/wvar_graph.png" width="80%"/>
</div>
...
Let $Y_t$ where t $\in$ $\{1,\ldots, T\}$ be a univariate time series.

Define $J = log_2(T)-1$,  $j \in \{1, \ldots, J\}$ and $\tau_j = 2^j$
...
plot(x = 10^seq(5), y = mat_mean_perf[,2], 
     log="x", type = "b", xlab = "Sample size", ylim =c(0, 2*10e7),
     cex.lab = 1.2, las = 1, 
     cex.main = 1.5,cex.axis = 1.1,
     col = "darkblue", pch = 16, ylab ="")
mtext(side = 2, text = "Mean execution time (microseconds)", line = 5.6, cex= 1.1)
grid(col="grey80", lty=1)
polygon(x = c(10^seq(5), rev(10^seq(5))), y = c(mat_mean_perf[,3], rev(mat_mean_perf[,5])), col = "#FF8C0019" , border = NA)
polygon(x = c(10^seq(5), rev(10^seq(5))), y = c(mat_mean_perf[,4], rev(mat_mean_perf[,6])), col = "#00008B19", border = NA)
