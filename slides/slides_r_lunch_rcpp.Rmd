...
Hence, there exists different way to script a function in `R`, and depending on the functions and objects defined, different implementation will have different run-time.
...
TO illustrate this notion, let us consider a simple example where we want to construct a vector of length $n$ with the sequence of $\mathbb{N}$ from 1 to $n$.
...
The equivalent of [base](https://stat.ethz.ch/R-manual/R-devel/library/base/html/base-package.html) `R` in `C++` is the `C++` Standard Library (`std`). `std` is a collection of classes and functions written in the core language. One can use functions from `std` in a `Rcpp` script by specifying the prefix `std::` before a function.
...
A simple algorithm to compute an estimator of the Wavelet Variance provide an interesting use case as it provide an example on how to mix the use of different `Rcpp` functions and `arma::` classes.

Algorithm

Let $Y_t$ where t $\in$ $\{1,\ldots, T\}$ be a univariate time series.

Define $J = log_2(T)-1$ and scale $\tau_j = 2^j$ where $j \in \{1, \ldots, J\}$ 

For each scale $\tau_j$:

$\hspace{.8cm}$ Compute the Wavelet coefficients $W_{t,\tau_j}$ defined as 
