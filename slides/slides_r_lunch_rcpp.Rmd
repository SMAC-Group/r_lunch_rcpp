...
@Sam for an introduction to xaringan, check later slides or see
...
also, check about infinite moon reader for development
...
- Understand how to use `C++` libraries for scientific computing (`Armadillo`)
...
# Motivations: Why using `C++` in `R`

When working with complex computations in `R`, it is often the case that `R` is simply not fast enough. In such cases, it is interesting to identify where are the bottleneck in a given code and to rewrite chunks of code that compose most of the computing time required by an expression in a flower-level language.

`Rcpp` makes it very easy to connect `C++` code to `R`.  
...
When working on the efficiency of an `R` code it is important to identify where the co
...
Before implementing a given code chunk that constitutes a bottleneck, one should try to write the `R` code as efficiently as possible. There exists multiple way to improve the performance of an `R` code. General advice are for example to declare type and length of data structure explicitly rather then appending to existing data structure.

Also, `R` is know to be particularly inefficient with iterative loop such as `for` and `while` loop. When computing operations on specific data structures, it is recommended to use the `apply()` family of functions rather than `for` loops. This family of function allow to manipulate slices of data structure in a repetitive way rather than by explicit for loops.
...
One can then proceed to a benchmarking of this function with:
```{r, cache = T}
microbenchmark::microbenchmark(fibo_r(20))
```
...
---
# A brief introduction to `C++` and `Rcpp` synthax
...
# Including `Rcpp` code in `R`

There exists two possibilities for sourcing `C++` code in `R`. One can directly source a `C++` function inside of an `R` code with "inline sourcing" with the function: 
- inline
...
---
# Including support for `Armadillo`

In addition to the functions provided by `Rcpp` and the `C++` standard library `std::`, one can also make use of high quality `C++` linear algebra libraries such as `Armadillo` and `Eigen` with respectively `R` package `RcppArmadillo` and `RcppEigen`. We present here how to use the `Armadillo` library.
...
This statistical quantiy is used in the context of inference foor multivariate times series.
...
.white[`r icons::fontawesome("link")` SMAC-Group/r_lunch_rcpp]
</a>
...
## Installing `r icons::fontawesome("download")`
...
## Why xaringan/remark.js? `r icons::fontawesome("check-circle")`
...
## Why **not** xaringan/remark.js? `r icons::fontawesome("times-circle")`
...
## Content boxes `r icons::fontawesome("box", )`
...
## Quotes `r icons::fontawesome("quote-right")`
...
## Lists, increments and footnotes`r icons::fontawesome("list")`
...
## icons `r icons::fontawesome("exclamation")`

Perhaps the easiest way to include icons in your xaringan presentations is through the **icons** package. The **icons** package lets you include [font awesome](http://fontawesome.io), [academicons](http://jpswalsh.github.io/academicons/) and [ionicons](http://ionicons.com) into R Markdown documents.
...
For more info see [the `r icons::fontawesome("github")` repo](https://github.com/ropenscilabs/icons).

]

---

## Data tables `r icons::fontawesome("table")`
...
## FAQ `r icons::fontawesome("question-circle")`
...
## Two columns! `r icons::fontawesome("columns")`
...
## R code and highlighting `r icons::fontawesome("r-project")`
...
## R code and plot output `r icons::fontawesome("r-project")`
...
## Citations and references `r icons::fontawesome("book")`
